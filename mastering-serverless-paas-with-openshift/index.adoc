...
* Firefox 17 or higher, or Chromium / Chrome or Safari
+
[NOTE]
...
. Under *All Services -> Red Hat Summit 2018*, select *Mastering Serverless PaaS with OpenShift*.
...
A full new Lab environment is deployed on every request. To make the environment unique a 4 character identifier is assigned to it (i.e. `1e37`), this identifier is referred in this documentation as *YOUR-GUID*.  

The Lab environment consists of the following architecture:

image::lab-architecture.png[Lab Architecture]

. You will access the lab from your Workstation aka "Summit Workstation"

. From your Workstation you will access the Lab Workstation machine to create and publish your functions
...
OpenWhisk will run as containerized inside OpenShift, from official image on https://hub.docker.com/u/openwhisk/[Docker Hub], properly tagged for OCP (rhdemo-6ee19949)
...
* Example for user 'bbking' and GUID '1e37', using the default ssh private key:
+
----
$ ssh -i ~/.ssh/id_rsa bbking-redhat.com@workstation-1e37.rhpds.opentlc.com
----

. From a web browser, open URL below in its own window or tab, using `developer` for the username and `rhsummit2018` for the password:

* *OpenShift console:* `https://infranode-<YOUR-GUID>.generic.opentlc.com:8443`
...
----
ssh <YOUR-OpenTLC-USERNAME-redhat.com>@workstation-<YOUR-GUID>.rhpds.opentlc.com
----
...
$ oc get pods
..
wskinvoker-00-4-prewarm-nodejs6              1/1       Running     0          5m
wskinvoker-00-5-prewarm-nodejs6              1/1       Running     0          12s
----

image::nodejspod.png[OpenWhisk JavaScript Runtime as NodeJS Pod]

Looking at previous action invokation output we can see some useful information in JSON format:
...
----
$ wsk -i action create hellosummit hellosummit.js
----
...
Update generated FunctionApp class in `$HOME/faas-lab/hello-summit/src/main/java/com/example/FunctionApp.java` with this code:
...
Update the Test as well in `$HOME/faas-lab/hello-summit/src/test/java/com/example/FunctionAppTest.java` with this code:
...
----
$ wsk -i action create hello-summit $HOME/faas-lab/hello-summit/target/hello-summit.jar --main com.example.FunctionApp
----
...
[NOTE]
If you want to modify again the code and update your actions, you can do it using:
wsk -i action update <action-name>
...
In the same way we did before, let's modify our Java function in `$HOME/faas-lab/hello-summit-web/src/main/java/com/example/FunctionApp.java`
...
and also the Test in `$HOME/faas-lab/hello-summit-web/src/test/java/com/example/FunctionAppTest.java`  as follows:
...
----
$ cd $HOME/faas-lab/hello-summit-web
$ mvn clean package
----
...
----
$ wsk -i action create --web=true hello-summit-web $HOME/faas-lab/hello-summit-web/target/hello-summit-web.jar --main com.example.FunctionApp
----
...
$ curl -sk $WEB_URL
$ curl -sk -X POST -H 'Content-Type: application/json' -d '{"name": "test"}' $WEB_URL
----
...
Get fired Action's activation id from this list:

----
$ wsk -i activation list --limit 1 hellosummit
----
